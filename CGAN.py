...
class CGAN(object):
    ...
    def __init__(self, sess, epoch, batch_size, z_dim, dataset_name, checkpoint_dir, result_dir, log_dir, learning_rate, disc_iter):
        self.sess = sess
    ...
    def visualize_results(self, idx):
        tot_num_samples = min(self.sample_num, self.batch_size)
        ...
        inc_score_curr, inc_score_new_curr, f_distance_curr, w_distance_curr = self.sess.run([inc_score, inc_score_new, f_distance, w_distance],
                                   feed_dict={self.z:z_sample, self.y: y_one_hot, self.inputs: self.data_X[0: self.batch_size]})
        ...
        save_images(samples[:image_frame_dim * image_frame_dim, :, :, :], [image_frame_dim, image_frame_dim],
                    check_folder(self.result_dir + '/' + self.model_dir) + '/' + self.model_name + '_epoch%03d' % idx + '_test_all_classes.png')
        ...
        for l in range(self.y_dim):
            ...
            samples = self.sess.run(self.fake_images, feed_dict={self.z: z_sample, self.y: y_one_hot})
            save_images(samples[:image_frame_dim * image_frame_dim, :, :, :], [image_frame_dim, image_frame_dim],
                        check_folder(self.result_dir + '/' + self.model_dir) + '/' + self.model_name + '_epoch%03d' % idx + '_test_class_%d.png' % l)
        ...
        save_images(canvas, [n_styles, self.y_dim],
                    check_folder(self.result_dir + '/' + self.model_dir) + '/' + self.model_name + '_epoch%03d' % idx + '_test_all_classes_style_by_style.png')
